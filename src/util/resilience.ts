/**
 * 端点级韧性（2026-09-11）：**分端点观测**（Step 0）+ **熔断**（Step 1）。
 *
 * 【为什么有这一步】此前超时治理「有界但不自适应」：每个工具一个固定墙钟，到点即降级，但**没有跨调用的
 * 健康状态**。桥接挂死时，每个后续病例都要各自再付一次 300s 才拿到同样的结论——一轮 101 例就是 101×300s。
 *
 * 【三条设计取舍，都是刻意的】
 *  1. **只动失败路径**。成功路径的预算与语义一字不改 → 跑次之间仍可比（评测靠锁 env 快照，
 *     浮动预算等于把「同配置可复现」的地板抽掉）。自适应墙钟已明确不做，理由见方案 Step 3。
 *  2. **连接类失败 ≠ 超时类失败**。
 *     - 连接被拒/重置：**关于服务健康的无歧义证据** → 直接记服务级熔断。
 *     - 超时：**模糊证据**（可能是这张巨片慢，不是服务死了）→ 只记 `(端点, slide)` 级；
 *       只有**跨片子**在窗口内累计够多张都超时，才升级为服务级。否则一张病态巨片能把整个服务判死、
 *       误伤后面所有病例（[[pathask-bridge-singlelevel-latent-bug]] 里 4.95GP 冷重算 ≈205s 正是这种形态）。
 ...
 *
 * 【与 Step 2（重试预算）的关系】两者**独立**，各做一件事，不互相触发：
 *  - 熔断负责「别再付墙钟」（跨病例、进程级）；
 ...
 *  所以刻意写成字面量而**不是**由标签拼出来（第一版就是拼的：`Patho-R1 vLLM` + `不可用` 少了空格
 *  → 词表不匹配 → 分类器落到 UNKNOWN。探针 ⑥ 抓到的就是这个）。 */
