 ...
 * 循环治理：台账类型 + 汇总读出口。
 *
 * `loopGuardSummary()` 是落盘字段的唯一数据源。台账（`LoopLedger`）由 `guardedExecute`
 * 逐条追加；在此之前 `session.loopLedger` 为 undefined，本函数返回 null，落盘字段即为 null/[]。
 */
...
/** 一次工具调用的台账行。agent 与 programmatic 两种 origin 都记（后者不计重复/停滞）。 */
export interface LoopCallRecord {
   ...
   *  32 位哈希会碰撞，而碰撞的后果是「把一次合法调用误判成重复」。 */
  fingerprint: string
  ...
  blocked?: boolean
  /** 被拦的原因。三种门禁互斥，取**先命中**的那个（顺序：步数 → 预算 → 重复）。
   *  这既是给分析用的诊断字段——只看 `blocked` 布尔值分不出「拦错了」与「拦对了」。 */
  ...
  /** 墙钟耗时 */
  ms?: number
  /** 结果文本摘要。**唯一用途**：同参数调用被重复门禁拦下时，把「你上次拿到的答案是这句」
...
/** 懒初始化台账（由 guardedExecute / runQuestion 调用）。 */
...
/** 开新问题前清台账。**必须清**，否则第二个问题的第一次调用就会因为第一个问题
...
/** 同一调用再次出现时怎么办。
 *  - `block`：确定性工具同参必然同果 → 重复纯属浪费（直接拦截）
 *  - `hint`：**非确定性**工具（VLM 采样），重复采样有诊断价值 → 永不拦，只提示（仅提示）
...
export const REPEAT_POLICY: Record<string, RepeatPolicy> = {
  ...
  counterfactual: 'exempt', // 新鲜度重跑是设计
...
/** 工具元数据登记簿。**为什么需要**：`beforeToolCall` 门禁要对一个**还没执行过**的调用
 ...
 *  后者没执行过、没产生证据也非模型之过。`exempt` 工具同理（终态动作、新鲜度重跑）。 */
 ...
 *  把它算成振荡会让「重复率」与「振荡率」两个指标互相污染，且提示文案会说错话。 */
 ...
 *  为什么不是「streak >= K 的步数」：那样一个长空转会被计成 7、8 次，而每一次触发都要
 *  投递一条 steer——计数即投递次数，语义必须是「这段空转**开始**了」。
 *
 *  ⚠️ 边沿条件是 `cur >= K && agentSteps >= minSteps` **且本段尚未触发过**，不是 `cur === K`。
 *  `cur === K` 只在「恰好越过 K 的那一步」为真：若那一步被 `minSteps` 挡下
...
export interface BudgetStatus {
  ...
  vlmBudget: number
  vlmRatio: number
  /** 是否已越过软阈（用它决定「要不要往 prompt 里加一行」） */
  soft: boolean
  /** 越界的维度名；空数组 = 都在预算内 */
  over: ('tokens' | 'vlm')[]
}

/** 预算只需要 session 上这两样东西。抽成具名接口是因为它有两个消费者
 *  （状态行、门禁），两处各写一遍结构类型就会各漂各的。 */
 ...
 *
 *  三条设计约束：
 *  ① **未过软阈时返回 null**——多数病例的 prompt **逐字不变**，把「不改」当成默认；
 *  ② 只给数字与出口，**不给禁止**（「不要调用 X」是硬阈的事）——软阈的价值是让模型
 *     自己决定收尾，而不是被剥夺工具；
...
// （在 `prepareToolCall` 内部、**参数校验之后**）。这一条位置事实带来三个结论，
// 每一条都直接决定下面的写法：
//  ① **程序化调用（`runSystemTool`）永远不会被拦**——它根本不走框架，
//     不需要在门禁里判 origin。
//  ② **参数没通过校验的调用到不了这里**，所以 `ctx.args` 一定是已解析的静态类型——指纹可以直接算。
//  ③ 被拦的调用**仍然会 emit `tool_execution_start`**（在 `prepareToolCall` 之前），
//     所以「拦下」不会改变 `tool_count` 的口径。
//
// **批处理顺序**：整批调用先**顺序**过门禁，执行 thunk 直到 `Promise.all` 才跑。
...
/** 工具调用总数的**默认**硬上限（步数）。这是防御性上限，正常病例永不触发。 */
export const DEFAULT_MAX_TOOLS = 15

/** 干预总开关。关闭即回到无治理的行为（连软阈状态行也一起关——状态行本身就是干预）。
 *
...
/** 步数硬上限。`PATHASK_MAX_TOOLS` 优先，**回落兼容旧 `MAX_TOOLS`**。
 ...
 *  这与 `REPEAT_POLICY` 未登记工具默认 `hint`（= 放行）方向相反，因为两件事的
...
/** 造一组治理钩子。三条钩子共享同一本台账、同一套阈值，**判据只算一份**。
 *
