/**
 * Agent 工厂：把 Agent 的构造**收敛到一处**，并挂上治理钩子。
 *
 * **为什么必须抽工厂**：治理钩子（`transformContext` / `beforeToolCall` / `shouldStopAfterTurn`）
 * 都挂在 `new Agent({...})` 的 config 上，而 Agent 在**调用方脚本**里构造。
 * 不收敛的话，治理层就只是**评测侧补丁**：生产路径上什么治理都没有。
 * 收敛之后「评测里跑的东西 = 产品里跑的东西」，这是本文件存在的全部理由。
 *
 * **钩子分工**（四条都是框架既有公开能力，不改 vendored `pi-agent`）：
 *  - `transformContext`：每轮刷新，**不进 transcript**（框架只把它当
 *    本轮的 messages 用，不写回 `state.messages`）→ 天然免去重，适合放「预算/停滞」这类
 *    每轮都该是最新数字的状态行。
 *  - `beforeToolCall`：**唯一的执行前门禁**。位置事实见 `governance.ts` 拦截节顶部。
 *  - `agent.steer()`：一次性投递（框架在每轮 turn_end 后 drain）→ 停滞/振荡/硬预算提示。
 *    **不是 `getSteeringMessages`**：`AgentOptions` 里没有这个字段，Agent 把它写死成 drain 自己的队列。
 *  - `shouldStopAfterTurn`：优雅退出 → 撞硬预算且本轮没走到收尾时收场。
 *  - `afterToolCall`：**永远不接**。它一旦 throw 会丢掉真实 tool result。
 *
 * `governance: false`：关闭全部治理钩子，行为**逐字相同**（直连框架、不受任何治理层影响）。
 * **`PATHASK_LOOP_GUARD_ENFORCE=0` 是另一件事**：它关的是「干预」——两处合起来才是 `enabled`，
 * 但仍共用同一个工厂，所以「评测里跑的 = 产品里跑的」不因开关而破。
 */
...
export interface CreateAgentOpts {
  ...
  tools: AgentTool<any>[]
  /** 关闭全部治理钩子。**默认开**——关掉的是钩子，不是台账：
   *  台账在工具层（`guardedExecute`），与这里无关，故观测数据照样落。 */
  governance?: boolean
  /** 覆盖 system prompt */
  systemPrompt?: string
  /** 覆盖流式函数。缺省 `siliconflowStreamFn`。 */
   ...
   *  **显式传值优先于 env**：要能在一个进程里用不同上限跑多次（`PATHASK_LOOP_GUARD_ENFORCE`
...
/** 构造 PathAsk 的 Agent。治理钩子默认挂上；`governance:false` 时行为**逐字相同**。 */
export function createPathAskAgent(opts: CreateAgentOpts): PathAskAgent {
  ...
  // `governance:false` 与 `PATHASK_LOOP_GUARD_ENFORCE=0` 都要求「什么钩子都不挂」，
  // 但两者的**理由不同**，别合并成一句注释。
  ...
  if (enabled) {
    ...
    // 门禁 + 一次性提示 + 收尾保险丝。**引用解析器现建**（`makeRefResolver` 查的是 session 上的
    ...
    const guard = createLoopGuard({
      ...
      // 工具调用才会真的碰它，那时 Agent 早已构造完毕。`?.` 不是防御性编程——只取 `guard`
