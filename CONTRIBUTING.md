...
感谢你的兴趣。先说清两点，它们决定了哪些改动是合适的：

- 本仓库是**研究级参考实现**，不是医疗器械，不宣称临床可用，也不越过病理医生。
- 本仓库**只发代码、不发数据**——不含数据集、切片、向量库、权重与评测 harness
  （数据面的边界与理由见 [`THIRD_PARTY_NOTICES.md`](THIRD_PARTY_NOTICES.md)）。
...
本来就在跑降级路径——离线桩没有端点，工具走的就是降级分支，且最终诊断不具临床意义。
